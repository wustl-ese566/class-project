...
// Intel processor and other low-end computer user have to reverse header byte
...
ImgArr read_Img(const char* filename) //load the image
{
	...
	int n_rows = 0;  
	int n_cols = 0;  
	//get the charactor with size of sizeof(magic_number) to &magic_number
	fread((char*)&magic_number,sizeof(magic_number),1,fp); 
	magic_number = ReverseInt(magic_number);  
	//get the size of image that for traing or testing, number_of_images
	fread((char*)&number_of_images,sizeof(number_of_images),1,fp);  
	number_of_images = ReverseInt(number_of_images);    
	//get the height of image that for traing or testing, Heigh
	fread((char*)&n_rows,sizeof(n_rows),1,fp); 
	n_rows = ReverseInt(n_rows);                  
	//get the width of image that for traing or testing, Width
	fread((char*)&n_cols,sizeof(n_cols),1,fp); 
	n_cols = ReverseInt(n_cols);  
	//get the ith image, saving it in the vec
	int i,r,c;

	//Initialize the matrix of image
...
LabelArr read_Lable(const char* filename)//load the image
{
	FILE  *fp=NULL;
	fp=fopen(filename,"rb");
	if(fp==NULL)
...
char* intTochar(int i)// transfer the number to character string
{
	int itemp=i;
	int w=0;
	while(itemp>=10){
...
char * combine_strings(char *a, char *b) // combine two character string
{
	char *ptr;
...
void save_Img(ImgArr imgarr,char* filedir) // save the image data to a file
{
